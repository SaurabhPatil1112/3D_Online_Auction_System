    ...
    if request.method == 'POST':
        ...
        if  image:
            ...
            listing = AuctionListing.objects.create(
                name=title, category=category, date=timezone.now(), startBid=startBid, description=description, user=user,image=image,vrmodel=vrmodel, active=True)
            listing.save()
            return HttpResponseRedirect(reverse("index"))
        else:
...
def vrmodel(request):
    #threedmodel = AuctionListing.objects.get()
    context={
        'vrmodel':vrmodel,
    }
    return render(request,"auctions/3djs.html",context)
